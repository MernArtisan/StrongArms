                                    ...
                                    <tbody>
                                        @foreach ($services as $u)
                                            <tr>
                                                ...
                                                <td>
                                                    @if($isAdmin) 
                                                    <a href="{{ route('service.edit', $u->id) }}" class="btn btn-info">Edit</a>
                                                    @else
                                                    @can('service-edit')
                                                    <a href="{{ route('service.edit', $u->id) }}" class="btn btn-info">Edit</a>
                                                    @endcan
                                                    @endif
                                                    @if($isAdmin) 
                                                    <form action="{{ route('service.destroy', $u->id) }}" method="POST" style="display:inline;" onsubmit="return confirm('Are you sure you want to delete this user?');">
                                                        @csrf
                                                        @method('DELETE')
