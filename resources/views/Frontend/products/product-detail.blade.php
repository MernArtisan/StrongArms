                    ...
                    <div class="products-photo">
                        ...
                        <div class="tab-content">
                            @foreach($product->images as $key => $img)
                                <div role="tabpanel" class="tab-pane fade {{ $key == 0 ? 'show active' : '' }}" id="img{{ $key }}">
                                    <img src="{{ asset($img->image) }}" alt="" style="width: 100%; height: 400px; object-fit: cover; border-radius: 10px;" />
                                </div>
                            @endforeach
                        </div>
                        <!-- Nav tabs -->
                        <ul class="nav nav-tabs" role="tablist">
                            @foreach($product->images as $key => $img)
                                <li role="presentation" class="nav-item">
                                    <a href="#img{{ $key }}" role="tab" data-bs-toggle="tab" class="nav-link {{ $key == 0 ? 'active' : '' }}">
                                        <img src="{{ asset($img->image) }}" alt="" style="width: 60px; height: 60px; object-fit: cover; border-radius: 5px;" />
                                    </a>
                                </li>
                            @endforeach
                        </ul>
                        ...
                        <div class="select-pro">
                            <form method="POST" action="">
                                ...
                                <div class="input-group">
                                    <input class="form-control w-25" type="number" name="quantity" value="1" min="1">
                                </div>
            ...
            <div class="row">
                <div class="col-md-12 col-sm-12">
                    <div class="section-heading">
                        <h2>Our Products</h2>
                        <p>All modern weapons can appreciate our broad services akshay handge pharetra, eratd fermentum feugiat, gun are best velit mauris aks egestasut aliquam.</p>
                    </div>
                </div>
            </div>
            <div class="row">
