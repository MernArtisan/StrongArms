...
<style>
    ...
    color: white;
    background-color: #222; /* optional: set background */
    border: 1px solid #555; /* optional: for visible border */
}
                            ...
                            <div class="price-range">
                                <div class="range">
                                    ...
                                    <div class="d-flex justify-content-between align-items-center">
                                        <input type="number" id="min_price" name="min_price" class="form-control" style="width: 45%;" value="{{ request('min_price', 0) }}">
                                        <span class="px-2 text-white">To</span>
                                        <input type="number" id="max_price" name="max_price" class="form-control" style="width: 45%;" value="{{ request('max_price', 10000000) }}">
                                    </div>
                                </div>
                            ...
                            @foreach ($products as $product)
                                <div class="col-sm-4 products mb-4">
                                    <figure style="width: 100%; height: 200px; overflow: hidden;">
                                        <a href="{{ route('productview.productdetail', $product->slug) }}">  <img src="{{ asset($product->image) }}" alt="" style="width: 85%; object-fit: cover;"></a>
                                    </figure>
                                    <div class="contents">
                                        <h3><a href="{{ route('productview.productdetail', $product->slug) }}" style="text-decoration: none;color: white">{{ $product->name }}</a></h3>
                                        <p>{{ $product->description }}</p>
                                        <span>${{ $product->price }}</span>
                                        {{-- <a href="" class="btn4">Add To Cart</a> --}}
                                        <button class="btn4 add-to-cart" data-id="{{ $product->id }}">Add to Cart</button>
...
<script>
    ...
    $('.add-to-cart').click(function () {
            ...
            $.ajax({
                ...
                // Delegate click for dynamically added .remove buttons
